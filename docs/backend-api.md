...
// Response 200
{
  "plan": "paid",
  "status": "active",
  "features": ["official_profiles", "auto_update", "detail_traversal"],
  "maxProfiles": 9999,
  "maxDevices": 3,
...
`plan` は `"free" | "paid"` の 2 値。JWT 内の features は短命キャッシュなので、UI 側で表示する正本としてこの API を使う。
...
### 6.1 webhook 配信は拡張側でユーザー設定の URL に直接送る

サーバを経由しない（ClipLayer 側に送信内容を保管しない方針）。設定は profile.delivery.webhookUrl。

抽出データをサーバに送る API は MVP では提供しない。AI 解析・クラウド同期は仕様撤回済み。

---

## 7. プロファイル作成代行（補助収益）

### 7.1 POST /profile-requests

ユーザーから profile 作成依頼を受け付ける。詳細仕様は [subscription.md](subscription.md) §13。
...
// Request
{
  "targetUrl": "https://...",
  "fields": [
    { "label": "顧客名", "description": "リスト行の左端の名前", "sampleValue": "山田太郎" }
  ],
  "delivery": { "type": "webhook", "url": "..." },
...
{
  "requestId": "uuid",
  "estimatedCompletion": "2026-05-23T00:00:00Z",
  "checkoutUrl": "https://checkout.stripe.com/...",
  "priceJpy": 2000
}
```

決済は Stripe Checkout（単発）で完成後に課金。

### 7.2 GET /profile-requests/:id
...
{
  "requestId": "uuid",
  "status": "pending" | "in_progress" | "ready_for_review" | "delivered" | "rejected",
  "deliveredProfileId": "user-profile-ulid" | null,
  "rejectReason": "..." | null
}
```
...
| FEATURE_NOT_IN_PLAN | 403 | plan に含まれない機能（Free が公式 profile を要求した等）|
...
| PROFILE_REQUEST_INVALID | 400 | 代行依頼の必須項目不足 |
...
`plan` は `"free" | "paid"` の 2 値。short-lived（15 分）なので features キャッシュとして使ってよい。
